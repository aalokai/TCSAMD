...
The RGB branch is trained as a structural defect detector initialized from a lightweight pretrained model.
```
...
Mean average precision summarizes detector quality across classes and IoU thresholds.
```
...
The telemetry stream is treated as a frame-level context source. Four engineered terms are used.
...
If the telemetry stream contains anomaly labels, a frame-level classifier can be trained directly.
```
...
Each detector output is reduced to a compact event table containing the top-confidence class and a modality score.
```
...
The final stage presents a frame-ordered replay of a selected mission. For each step, the notebook shows RGB evidence, thermal evidence, telemetry context, and the fused alert.
```
